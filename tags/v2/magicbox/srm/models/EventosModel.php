...
class EventosModel
{
	...
	public function eliminarParticipantesEvento($eve_id) 
	{
		$categorias=array();
		try{
			echo $consulta="delete from par_eve where PE_FK_EVE_ID=".$eve_id;
			$this->db->exec($consulta);
			$retorno='true';
		}		
		catch(PDOException $e)
	...
	public function ingresarParticipantesEvento($eve_id,$participantes)
	{
		try{
			...
			$consulta="INSERT INTO participante (PAR_ID,PAR_NOMBRE) VALUES (AUTO_INC_participante.NEXTVAL,'".$par_nombre."')";
			$this->db->exec($consulta);
			$retorno='true';
		}		
		catch(PDOException $e)
